public class FurtherInst extends mTSP implements BestPrice, BestNode{

    public FurtherInst(int[][] cost, String[] cities, int start) {
        super(cost, cities);
        this.start = start;
    }
    ...
    public int findNode() {
        ...
        for (Integer integer : tour) {
            for (int j = 0; j < cost.length; j++) {
                if (cost[integer][j] > max && visited[j] != 1) {
                    max = cost[integer][j];
                    node = j;
                }
            }
    ...
    public String toString(){
        return "FurtherInst " +getTotalCost()+ " " +tour;
    }
}
